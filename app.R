...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("RESULTADOS", h3(img(src = "JM.jpg", height = 85, width = 167)) ,DT::dataTableOutput("ziptable1")),
    tabPanel("SECCION", h3(img(src = "JM.jpg", height = 85, width = 167)) ,DT::dataTableOutput("ziptable2")),
    tabPanel("CASILLAS", h3(img(src = "JM.jpg", height = 85, width = 167)) ,DT::dataTableOutput("ziptable3"))
  ))
...
server <- function(input, output) {
  ...
  output$map_1 <- renderLeaflet({
    tm <- shp_seccion %>% 
      ...
      tm_shape() +
      tm_polygons(col = "MORENA", id = "SECCION", midpoint = 60   , n = 5, style = "pretty", alpha = .5, palette = "YlOrRd", popup.vars = c("%JM" ="MORENA", "JULIO M" = "JULIO MENCHACA", "CAROLINA" = "CAROLINA V", "FRANCISCO" = "FRANCISCO X", "JOSE LUIS" = "JOSE LUIS L")) +
